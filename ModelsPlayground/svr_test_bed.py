...
def svr_test_bed(data: pd.DataFrame, test_specification: pd.DataFrame):
    ...
    for index, test in test_data.iterrows():
        ...
        print(
            f"Test {index + 1} of {len(test_data)} with sample_size {sample_size}, test_size {test_size} and features {features}"
        )
        ...
        test_data.at[index, "time_setup"] = time.strftime(
            "%H:%M:%S", time.gmtime(time_setup)
        )
        test_data.at[index, "time_fitting"] = time.strftime(
            "%H:%M:%S", time.gmtime(time_fitting)
        )
        test_data.at[index, "time_pred"] = time.strftime(
            "%H:%M:%S", time.gmtime(time_pred)
        )
